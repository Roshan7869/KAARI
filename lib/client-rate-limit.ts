...
/**
 * Client-Side Rate Limiting Service (In-memory)
 *
 * ⚠️ IMPORTANT: This is client-only rate limiting, NOT distributed.
 * For production server-side rate limiting, use server-rate-limit.ts with Upstash Redis.
 *
 * SECURITY: Prevents brute force attacks on the client by:
 * - Tracking request counts per identifier in browser memory
 * - Implementing exponential backoff for repeated failures
 * - Blocking suspicious actions temporarily
 *
 * Limitations:
 * - Data is lost on page refresh (session-only)
 * - No protection against requests from other browsers/IPs
...
// ── Compatibility stubs for test environments ────────────────────────────────
// The old rateLimit.ts had a "fallback mode" concept for testing.
// The new implementation is always synchronous; these are no-ops.
export const enableFallbackMode = (): void => {}
export const resetFallbackMode  = (): void => {}
export const isFallbackMode     = (): boolean => false
