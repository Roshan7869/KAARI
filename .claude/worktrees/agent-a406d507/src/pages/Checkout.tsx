  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCod) {
        ...
        const returnUrl = `${window.location.origin}/order-confirmation/${result.order_id}`;
        const notifyUrl = `${window.location.origin}/api/payment-webhook`;

        let paymentUrl: string;

        try {
          // For UPI payments, use Cashfree SDK via dedicated Payment page
          if (formData.paymentMethod === 'upi') {
            const paymentSession = await createCashfreeOrder({
            ...
            // Redirect to Payment.tsx for UPI app selection and Cashfree SDK integration
            // This provides better UX with UPI app selection instead of Cashfree's hosted checkout
            paymentUrl = `/payment?session_id=${encodeURIComponent(paymentSession.payment_session_id)}&order_id=${encodeURIComponent(result.order_id)}&return_url=${encodeURIComponent(returnUrl)}`;
          } else {
            // For other payment methods (card, netbanking, wallet), use dummy payment
            const paymentSession = generateDummyPaymentSession(
              result.order_id,
              cart.pricing.total,
              formData.paymentMethod
            );
            paymentUrl = generatePaymentPageUrl(paymentSession.session_id, returnUrl);
          }
