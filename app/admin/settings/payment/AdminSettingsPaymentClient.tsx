...
export default function PaymentGatewaySettings() {
  ...
  useEffect(() => {
    if (existingConfig) {
      const maskedApiKey = existingConfig.api_key_masked || '';
      setConfig({
      ...
      setSecretPresence({
        ...
        hasApiSecret: existingConfig.has_api_secret,
        hasWebhookSecret: existingConfig.has_webhook_secret,
      });
      setIsEditing(false);
    }
  }, [existingConfig]);
  ...
  const saveMutation = useMutation({
    mutationFn: async (gatewayConfig: Record<string, string | boolean>) => {
      const res = await fetch('/api/admin/settings/payment', {
        method: 'POST',
  ...
  const handleSave = () => {
    ...
    const trimmedApiSecret = config.api_secret.trim();
    const effectiveApiKey = (!trimmedApiKey || trimmedApiKey.includes('••')) ? secretPresence.hasApiKey : true;
    const effectiveApiSecret = (!trimmedApiSecret || trimmedApiSecret.includes('••')) ? secretPresence.hasApiSecret : true;

    if (config.is_active && (!effectiveApiKey || !effectiveApiSecret)) {
    ...
    if (trimmedApiKey && !trimmedApiKey.includes('••')) payload.api_key = trimmedApiKey;
    if (trimmedApiSecret && !trimmedApiSecret.includes('••')) payload.api_secret = trimmedApiSecret;
    if (config.webhook_secret.trim() && !config.webhook_secret.includes('••')) {
          ...
          {!config.is_test_mode && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3">
              <p className="font-body text-sm text-red-700 font-medium">
                Warning: production mode will process real payments. Ensure your credentials are correct.
              </p>
            </div>
          )}
        </CardContent>
