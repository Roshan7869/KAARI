...
export default function FeatureFlagsPage() {
  ...
  const fetchFlags = useCallback(async () => {
    ...
    try {
      ...
      const payload = await res.json() as { flags?: FeatureFlag[]; error?: string };
      if (!res.ok) {
  ...
  const handleToggle = async (flag: FeatureFlag) => {
    ...
    try {
      ...
      const payload = await res.json() as { error?: string; flag?: FeatureFlag };
      if (!res.ok) {
      ...
      if (payload.flag) {
        setFlags((prev) =>
          prev.map((item) => (item.id === payload.flag!.id ? payload.flag! : item)),
        );
      }
      ...
      setError(`Failed to update "${flag.flag_label}": ${msg}`);
    }
