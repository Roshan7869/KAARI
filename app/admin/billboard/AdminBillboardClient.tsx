...
export default function AdminBillboardPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      try {
        const exclude = slots.map((s) => s.product_id).join(',');
        const params = new URLSearchParams({
          q: searchQuery.trim(),
          exclude,
          limit: '30',
        });
        const res = await fetch(`/api/admin/billboard/search?${params.toString()}`, { cache: 'no-store' });
        const payload = await res.json() as { products?: SearchProduct[]; error?: string };
        if (!res.ok) {
  ...
  async function handleCustomImageUpload(index: number, file: File) {
    ...
    try {
      ...
      logger.error('[Billboard upload]', err, { context: 'billboard-upload' });
    } finally {
              ...
              {!searching && searchResults.map((product) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-body font-medium text-sm text-foreground truncate">{product.title}</p>
                    <p className="font-display text-xs text-primary font-bold">
